...
internal static class AuthTestData
{
    ...
    public static AuthToken CreateEmailVerificationToken(
        DateTimeOffset? expiresAtUtc = null,
        string tokenHash = VerificationTokenHash) => AuthToken.CreateSingleUseToken(
        ActiveUser.Id,
    ...
    public static Mock<IJwtService> CreateJwtService()
    {
        ...
        service.Setup(candidate => candidate.GenerateEmailVerificationToken()).Returns(RawVerificationToken);
        service.Setup(candidate => candidate.HashEmailVerificationToken(RawVerificationToken))
