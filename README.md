...
| `Domain/` | Generated database POCOs, custom partial behavior, value objects, enums, and domain exceptions |
| `Infrastructure/` | Generated EF context, PostgreSQL account persistence, JWT, Redis, email, and other adapters |
...
Neon schema `public` is the source of truth; the API never applies migrations. After an approved database schema change, regenerate only the checked-in database model:
